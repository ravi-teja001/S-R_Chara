...
import { getUser as getStoredUser, setToken, setUser as setStoredUser, clear as clearAuthStorage } from '@/lib/authStorage';
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        // Railway API: restore user from stored token/user
        if (environment.apiUrl) {
          ...
          if (token && stored) {
            setUser({
              ...
              stockPointId: stored.stockPointId,
              plantId: stored.plantId,
            });
            console.log('✅ User restored from Railway session');
          } else {
            setUser(null);
          }
          setLoading(false);
    ...
    if (!environment.apiUrl && !environment.supabaseUrl.includes('placeholder')) {
  ...
  const resetPassword = async (email: string): Promise<{ success: boolean; message: string }> => {
    try {
      ...
      if (environment.apiUrl) {
        return { success: false, message: 'Password reset is not available with the current backend. Contact support.' };
      }
